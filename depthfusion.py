...
def ucsnet_to_gipuma_dmb(in_path, out_path):
    '''convert ucsnet .pfm output to Gipuma .dmb format'''
    
    image, _ = read_pfm(in_path)
    write_gipuma_dmb(out_path, image)
    return 

def ucsnet_to_gipuma_cam(in_path, out_path):
    '''convert ucsnet camera to gipuma camera format'''
...
def ucsnet_to_gipuma(dense_folder, gipuma_point_folder):
    ...
    for image_name in image_names:
        ...
        ucsnet_to_gipuma_cam(in_cam_file, out_cam_file)
    ...
    for image_name in image_names:
        ...
        ucsnet_to_gipuma_dmb(in_depth_pfm, out_depth_dmb)
...
def probability_filter(dense_folder, prob_threshold, s=3):
    ...
    for image_name in image_names:
        ...
        init_depth_map_path = os.path.join(depth_folder, 'dep_'+image_prefix+'_3.pfm')
...
if __name__ == '__main__':
    ...
    point_folder = os.path.join(dense_folder, 'points_ucsnet')
    ...
    print ('Convert ucsnet output to gipuma input')
    ucsnet_to_gipuma(dense_folder, point_folder)
